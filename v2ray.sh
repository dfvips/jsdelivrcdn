...
v2ray_conf_dir="/www/server/v2ray"
...
[[ -f "/www/server/v2ray/vmess_qr.json" ]] && mv /www/server/v2ray/vmess_qr.json $v2ray_qr_config_file
...
v2ray_install() {
    ...
    if [[ -d /www/server/v2ray ]]; then
        rm -rf /www/server/v2ray
    fi
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
...
menu() {
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/dfvips/jsdelivrcdn/master/v2ray.sh)
        ;;
    4)
